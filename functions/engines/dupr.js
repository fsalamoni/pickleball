/**
 * Motor de rating "estilo DUPR" no servidor — porta fiel de
 * `src/modules/rating/domain/duprScale.js`.
 *
 ...
 * implementações sobre as mesmas partidas e exige rating idêntico, casa
 * decimal por casa decimal. Se alguém mexer num lado só, o teste quebra.
 *
 * Qualquer mudança de regra aqui tem de ser feita nos DOIS arquivos.
 *
 * O texto abaixo é o do original, preservado de propósito:
 *
...
function reliabilityFromGames(gamesPlayed) {
  const g = Math.max(0, Number(gamesPlayed) || 0);
 ...
 */
function seedFromProfile(profile, levelTable = []) {
 ...
 */
function applyDuprMatch(state, match, options = {}) {
 ...
 */
function computeDuprRatings(matches, options = {}) {
