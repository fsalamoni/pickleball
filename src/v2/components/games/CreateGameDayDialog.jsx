...
import {
  GAME_DAY_FORMAT, GAME_DAY_FORMAT_LABELS, isPlayFormat,
...
/**
 * Diálogo para CRIAR um dia de jogo (público/privado).
 *
 * ⚠️ Ele não edita. Depois de criado, tudo o que se muda num dia de jogo —
 * nome, data, local, visibilidade, formato, quadras e quem organiza — vive no
 * cartão "Configurações do dia de jogo" (`GameDaySettingsCard`), que ABRE na
 * própria tela e é o MESMO no atleta, na arena e no clube. Dois lugares
 * editando o mesmo campo divergem, e um modal transforma "ajustar uma coisa"
 * num desvio para outra tela.
 */
export default function CreateGameDayDialog({ open, onOpenChange, onCreated }) {
  ...
  const [form, setForm] = useState(defaultsForNew);

  // Reabre sempre com os padrões do momento (cidade/UF do perfil).
  useEffect(() => {
    if (open) setForm(defaultsForNew());
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open, userProfile?.city, userProfile?.state]);

  // Formatos oferecidos na criação: fonte ÚNICA (`gameDayFormatChoices`).
  // Americano aprimorado, Mexicano e Rei da Quadra aparecem cada um com a sua
  // própria flag; o que já está no formulário nunca some da lista.
  const formatOptions = useGameDayFormatChoices({ current: form.format });
        ...
        <div className="space-y-4">
          ...
          {showFormatSelect && (
            <div>
              ...
              {isAmericanoLiveFormat(form.format) && (
