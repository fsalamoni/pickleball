...
const estado = { membros: [], pacotes: [], atletas: [] };
...
vi.mock('@/modules/arenas/hooks/useArenaV3', () => ({
  useArenaMembers: () => ({ data: estado.membros, isLoading: false }),
  useArenaPackages: () => ({ data: estado.pacotes, isLoading: false }),
...
async function render(rota = '/arenas/a1/gerir?aba=membros') {
  await act(async () => {
    root.render(
      <MemoryRouter initialEntries={[rota]}>
        <Routes>
          <Route path="/arenas/:arenaId/gerir" element={<><ArenaMembersPanel arena={{ id: 'a1' }} /><Onde /></>} />
        </Routes>
      </MemoryRouter>,
    );
  });
