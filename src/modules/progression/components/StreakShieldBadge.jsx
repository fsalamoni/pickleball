...
/**
 * StreakShieldBadge — visual do estado de proteção da streak.
 * Mostra grace days, freezes, vacation mode, comeback bonus.
 *
 * 4 estados:
 *  - grace (verde-água)
 *  - freeze (azul, quando há freezes)
 *  - vacation (amarelo, modo férias)
 *  - comeback (roxo, bônus de retorno)
 */
    ...
    return (
      <V2Surface className={className}>
        <div className="flex items-center gap-2 text-sm text-gray-500">
          ...
          <span>Carregando proteção de streak…</span>
        </div>
      </V2Surface>
    );
  }
    ...
    <V2Surface className={className}>
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1">
          <p className="text-xs font-bold uppercase tracking-wide text-gray-500">Proteção da Streak</p>
          <div className="mt-1 flex flex-wrap items-center gap-1.5">
            {grace > 0 && (
              <V2Badge tone="cyan" data-testid="streak-grace">
                <Sparkles className="h-3 w-3" /> {grace} grace
              </V2Badge>
            )}
            {freezes > 0 && (
              <V2Badge tone="blue" data-testid="streak-freeze">
                <Snowflake className="h-3 w-3" /> {freezes} {freezes === 1 ? 'freeze' : 'freezes'}
              </V2Badge>
            )}
            {onVacation && (
              <V2Badge tone="amber" data-testid="streak-vacation">
                <Umbrella className="h-3 w-3" /> Férias ativas
              </V2Badge>
            )}
            {comeback > 0 && (
              <V2Badge tone="purple" data-testid="streak-comeback">
                <Sparkles className="h-3 w-3" /> +{comeback} comeback
              </V2Badge>
            )}
            {grace === 0 && freezes === 0 && !onVacation && comeback === 0 && (
              <span className="inline-flex items-center gap-1 text-xs text-gray-500">
                <Lock className="h-3 w-3" /> Sem proteções ativas
              </span>
            )}
          </div>
        </div>
      ...
      {(onUseFreeze || onToggleVacation) && (
        <div className="mt-3 flex flex-wrap gap-2 border-t border-gray-100 pt-3">
          {onUseFreeze && freezes > 0 && (
            ...
            >
              <Snowflake className="h-3.5 w-3.5" /> Usar 1 freeze
            </button>
          )}
          {onToggleVacation && (
            <button
            ...
            >
              <Umbrella className="h-3.5 w-3.5" /> {onVacation ? 'Sair de férias' : 'Entrar de férias'}
            </button>
          )}
        </div>
      )}
