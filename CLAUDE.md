...
**"Vou mexer em torneio interno da arena"**  → ⚠️ **ele SAIU na Onda CB** (a tela; os dados ficam). Começar o torneio interno criava um dia de jogo da arena, e desde a Onda CA todo jogo aberto já é um — era uma segunda porta para a mesma sala. **Torneio da casa agora = torneio da PLATAFORMA com `arena_id`**, com as regras da plataforma. `arena_internal_tournaments` e `arena_ladders` seguem no banco, intactos: o ladder `{arena}_geral` é LIDO pelo ranking da casa ("torneios internos (formato antigo)"), os abertos aparecem na Central (`LegacyInternalTournamentsNotice`) para a arena cancelar avisando os inscritos, e o bloqueio de quadra dos antigos segue derivado (`tournamentBlocks`). **Nunca** crie tela nova que escreva nessas coleções. Os filhos `leagues_internal`/`leagues_ladder`/`leagues_prizing` viraram status `retired` (novo: não liberável, some da tela da arena). Ver `docs/24-MODULOS-DE-ARENA/06-TORNEIOS-INTERNOS.md` (topo) e `09-INTEGRACAO-NA-ARENA.md` §15
...
**"Vou mexer na indicação que chega com a reserva"** → o atleta digita o código no pedido (`BookingReferralField`, oferecido por `shouldOfferReferral`) e ele vai PENDENTE na primeira reserva (`arena_bookings.referral`). **Quem confere é a arena, na confirmação** (`applyBookingReferral`, `services/bookingReferralService.js`): o atleta não lê os códigos dos outros. Ali se credita cada lado, se aplica o desconto de quem chegou no valor acordado e se avisa os dois; recusada fica com o motivo, e nada disso derruba a confirmação. Reserva instantânea não passa pela confirmação: vai para "Indicações que chegaram com reservas" (aba Indicações) e para a faixa "Precisa de você". A regra só deixa o atleta criar `referral` pendente e só a arena alterá-lo — e a trava é só no UPDATE (no delete não há `request.resource`). Os códigos do atleta ficam no Perfil (`MyReferralCodes`). Ver `docs/24-MODULOS-DE-ARENA/03-MARKETING.md` §10
**"Vou criar um tipo de cupom / mexer em vale ou no controle de uso"** → o cupom tem `kind` em TRÊS famílias (`COUPON_KIND_META` em `arenas/domain/marketing.js`): **reserva** (desconto, hora grátis — entram no preço), **vale** (aula, clínica, comida, bebida, brinde, aluguel, evento, outro — registrados na recepção por `redeemVoucher`, numa transação) e **indicação** (as REGRAS do indique e ganhe; um programa ativo por arena). Sem `kind` é desconto — todo cupom antigo. O **custo interno do vale NUNCA vai no cupom** (legível por conta logada): mora em `arena_settings.coupon_costs` (`setCouponUnitCost`). O controle de uso é `couponUsageReport` (`domain/couponUsage.js`): número desconhecido é "—", nunca zero, e com uma consulta falhando o relatório não aparece. Código de indicação só vale se for **legítimo** (`isLegitReferral`: documento `{arena}_{uid}` do dono e código com o prefixo do uid). Componentes em `v2/components/arenas/marketing/coupons/`. Ver `docs/24-MODULOS-DE-ARENA/03-MARKETING.md` §9
**"Vou mostrar um CÓDIGO (cupom, promoção, indicação) numa tela"** → `CopyCodeButton` (`src/v2/ui/CopyCodeButton.jsx`): o código **é** o botão de copiar, com nome acessível e ✓. **Nunca** escreva `navigator.clipboard.writeText` à mão — `useClipboard` tem o caminho antigo para quando a API recusa, e a mensagem de erro traz o código. Para um CUPOM, use o tíquete `CouponArt` (`coupons/CouponArt.jsx`, com `copyable`): a arte (`arena_coupons.art`, cinco modelos em `domain/couponArt.js` + os da arena em `arena_settings.coupon_templates`, ou imagem enviada) e o código no canhoto. O código nunca vai dentro da imagem. Ver `docs/24-MODULOS-DE-ARENA/03-MARKETING.md` §13
**"Vou mexer em banner de campanha"** → ⭐ `docs/24-MODULOS-DE-ARENA/03-MARKETING.md` §12. Domínio em `arenas/domain/bannerArt.js` (os **cinco modelos**, congelados; a especificação do upload; contraste; os modelos da arena) e `campaignBanner.js` (destinos, estado no ar/pausado/encerrado, o que vai para a página e para a tela inicial). O desenho é UM componente, `BannerArt` (texto em `cqw`, que escala pela largura do PRÓPRIO banner), usado no editor, na página da arena, na tela inicial e na página da campanha — **nunca** desenhe banner de outro jeito. Quatro regras: (1) **os modelos da plataforma nunca mudam** — salvar cria modelo da arena (`arena_settings.banner_templates`, ids `arena:…`); (2) **destino é lista fechada** (`destinationLink`), nenhum link digitado, e só os dos módulos ligados; (3) a imagem enviada tem **descrição obrigatória** e só URL do Storage do projeto (`isAllowedImageUrl`); (4) o serviço **refaz a validação** antes de gravar. Os destinos usam as âncoras da página da arena — mais um motivo para nunca renomear um `id="arena-…"`
**"Vou mexer nas promoções da TELA INICIAL"** → é o cupom com `show_public` **e** `show_home` (a arena marca "Também como banner na tela inicial"), montado em `HomePromoBanners` atrás de `arena_modules`. O filtro é por **LOCALIDADE** (cidade/estado), não por distância: nem arena nem perfil têm coordenadas, e medir km exigiria geocodificar e gravar em todas as arenas. Domínio em `arenas/domain/homeBanners.js` (`resolveBannerRegion`, `homeBanners`, `bannerCities`); a escolha de região fica no navegador (`viewPreference`, `home:promocoes:regiao`). ⭐ **Sem cidade nem estado no perfil, NÃO mostre o Brasil inteiro** — era o que o pedido queria evitar; a tela pede a cidade. A consulta é só de igualdades (`show_home` + `active`, sem índice composto); falhando, a seção some. O carrossel pausa com foco/toque, tem botão de pausar e não gira com "menos movimento". Desde a Onda CC o **banner de campanha** (`show_home` + `banner_active`) entra no MESMO carrossel e filtro (`eligibleBanners` com `campaigns`); cada fonte falha sozinha, e com uma falhando a região vazia não afirma "não divulgaram". Ver `docs/24-MODULOS-DE-ARENA/03-MARKETING.md` §11 e §12
...
**"Vou acrescentar uma seção a uma central (arena, professor, torneio)"** → a barra é `V2SectionNav` + `V2SubTabs` (`src/v2/ui/V2SectionNav.jsx`) e **quebra em linhas, nunca rola para o lado** — com treze seções numa barra rolável, o que não cabia não era visto. Na arena, toda seção leva `grupo: 'atender'` (o trabalho do dia) ou `'gerir'` (como a arena é) em `buildArenaSections`: são as duas linhas com nome da barra. Ver `docs/24-MODULOS-DE-ARENA/09-INTEGRACAO-NA-ARENA.md` §14
...
**"Onde está o RANKING DA CASA? Como ele soma?"** → `arenas/domain/houseRanking.js` (conta, pura) + `hooks/useHouseRanking.js` (junta as leituras) + `v2/components/arenas/houseRanking/HouseRankingPanel.jsx` (o MESMO corpo na Central → Torneios → *Ranking da casa* e em `/arenas/:id/torneios`). Módulo `leagues` (agora "Ranking da casa"). Soma, por temporada: dias de jogo da arena com placar (colocação no RANKING DO DIA, `computeGameDayLeaderboard` — 100/70/50/35, jogou 10) + torneios da casa ENCERRADOS por categoria (o dobro; mata-mata pela FINAL, não por vitórias) + o ladder antigo. **DERIVADO, nunca gravado** — ranking gravado por navegador é o segundo escritor da Onda BJ. Por isso a conta NÃO roda na página da arena (todo visitante pagaria a temporada): lá fica só a chamada (`HouseRankingTeaser`), e a lista dos torneios importa de `domain/houseTournaments.js` (leve), nunca de `houseRanking.js` (puxa o motor). O dia de jogo de torneio interno ENCERRADO sai da soma pelo placar (`legacyLadderGameDayIds`) — já está no ladder. Leitura que falha vai para "Ficou de fora"; o vazio só é afirmado com tudo carregado
**"Vou listar os dias de jogo de uma arena"** → `listArenaGameDays`, e ela filtra `arena_id` **E** `visibility == 'public'`. 🐞 Só com `arena_id` a regra de `game_days` (criador / membro / público / clube) não é provável para a CONSULTA, e o Firestore a recusava para **todo mundo menos o admin da plataforma** — o gestor da arena inclusive. O admin testava com a conta de admin; para quem usa, a lista da Central, a seção da página da arena e o bloqueio derivado do calendário falhavam em silêncio. Todo dia de jogo de arena nasce público, então o filtro não esconde nada. Emulador em `tests/rules/houseRanking.rules.test.js`, guarda de fonte em `games/services/listArenaGameDays.guard.test.js`. **Lição geral**: teste consulta com uma conta que NÃO é admin — `isPlatformAdmin()` na regra esconde qualquer consulta mal filtrada
...
>   mas permitir que a arena também faça upload ou crie o seu próprio. E deve
>   ser possível copiar o código"*. Cada cupom é um TÍQUETE: a parte principal
>   diz o que ele dá, e o canhoto traz o código — sempre o de verdade, e
>   tocável para copiar (página da arena, tela inicial e a lista da Central).
>   **Cinco modelos** (Clássico, Neon, Quadra, Festa, Sol) com título e texto
>   em branco de propósito — em branco, valem o benefício e a descrição do
>   próprio cupom, então a arte nunca desmente o cupom; textos e cores
>   personalizáveis; **modelos da arena** em `arena_settings.coupon_templates`;
>   ou a **imagem enviada** (1200 × 600, sem o código escrito nela — ele fica
>   no canhoto e nunca desatualiza). O "copiar" virou uma peça só
>   (`CopyCodeButton`), com o caminho antigo quando a API do navegador recusa
>   e o código na mensagem de erro. Um campo opcional (`arena_coupons.art`),
>   zero regra; 8 cenários novos no emulador.
>
> - **Onda CC — Banners de campanha** (2026-09-25): *"banner específico da
>   campanha, com hiperlink para os detalhes da campanha ou para as funções
>   específicas… gerar o banner na própria plataforma, ou fazer upload… 5
>   modelos… as arenas devem poder salvar os seus próprios modelos"*. A
>   campanha ganhou BANNER: criado na plataforma a partir de **cinco modelos**
>   (Destaque, Oferta, Evento, Vitrine, Chamado — textos, cores, "usar a cor da
>   arena", prévia no computador e no celular) ou **enviado pronto**, com a
>   especificação ANTES do botão (1600 × 800, mínimo 1200 × 600, 2:1, área
>   segura desenhada) e a imagem conferida antes de subir. **Modelos da arena**
>   ficam em `arena_settings.banner_templates`, ao lado dos cinco, que nunca
>   mudam. O banner leva a um destino de uma lista FECHADA (página da campanha,
>   reservar, jogos abertos, um dia de jogo, um torneio, um produto — a loja
>   abre com ele em destaque —, planos, aulas, promoções, ranking) e aparece na
>   página da arena ("Em destaque") e, se a arena quiser, na tela inicial, no
>   mesmo carrossel e filtro de região das promoções, até uma data. Pausar e
>   retomar sem apagar; editar sem reenviar o aviso. Página "saiba mais" em
>   `/arenas/:id/campanhas/:cid`. Zero coleção, índice ou regra; campos
>   opcionais; 14 asserções novas no emulador.
>
> - **Onda CB — Torneios da casa e ranking da casa** (2026-09-25): *"os
>   torneios da plataforma viram os torneios da casa, excluindo o que foi
>   criado como torneio da casa"*. O torneio interno saiu da tela — começar um
>   criava um dia de jogo, e desde a Onda CA todo jogo aberto já é um. **Torneio
>   da casa = torneio da plataforma sediado na arena**, com as regras da
>   plataforma e "Criar torneio aqui". O que o interno tinha de bom virou o
>   **ranking da casa**: jogos abertos com placar (pela colocação no ranking do
>   dia) + torneios da casa encerrados (o dobro, e no mata-mata pela FINAL),
>   por temporada e por modalidade, **derivado** — soma sozinho, sem botão, sem
>   escrita; com "de onde vêm os pontos" e "o que ainda não entrou". Nada foi
>   apagado: o ladder antigo segue somando (sem contar duas vezes o dia de
>   jogo do torneio encerrado) e os torneios internos abertos aparecem na
>   Central para a arena cancelar avisando os inscritos. **🐞 E ninguém listava
>   os dias de jogo da arena**: a consulta só por `arena_id` era recusada pela
>   regra para todo mundo menos o admin da plataforma — o gestor inclusive.
>   Agora leva `visibility == 'public'`. Zero coleção, índice, regra ou função;
>   9 asserções novas no emulador.
>
...
>   a arena. Agora a arena pode marcar o cupom divulgado como **banner na tela
>   inicial**, e ele entra num carrossel (troca a cada 7 s, pausa, setas e
>   pontos com nome, respeita "menos movimento") com a marca da arena. O filtro
>   é por **localidade** (minha cidade, meu estado, outra cidade, Brasil todo),
>   porque arena e perfil não têm coordenadas; sem cidade no perfil, a tela
>   pede a cidade em vez de mostrar o país inteiro. Um campo opcional
>   (`arena_coupons.show_home`), zero regra, zero índice; duas asserções novas
>   no emulador provam a leitura da tela inicial.
>
> - **Onda BY — A indicação de ponta a ponta** (2026-09-25): o código de
>   indicação saiu da página de cada arena para o **Perfil** ("Meus códigos de
>   indicação", com as regras de cada arena), o pedido de reserva ganhou
>   **"Foi indicado por alguém?"** na primeira reserva, e a indicação passou a
>   ser **registrada sozinha quando a arena confirma**: o código é conferido
>   contra o banco e as regras, cada lado é creditado, o desconto de quem chega
>   entra no valor acordado e os dois são avisados. Recusada, fica com o
>   motivo. A reserva instantânea vai para a aba Indicações e para "Precisa de
>   você". Um campo opcional (`arena_bookings.referral`), e a regra só deixa a
>   arena decidir a indicação, com 8 asserções novas no emulador.
>
> - **Onda BX — Cupons com tipo, recepção de vales e controle de uso**
>   (2026-09-25): o cupom ganhou TIPO em três famílias — desconto e hora
>   grátis entram no preço da reserva; aula, clínica, comida, bebida, brinde,
>   aluguel e evento são VALES registrados na recepção; e a indicação virou um
>   cupom com REGRAS (quanto ganha cada lado, "só quem nunca reservou aqui",
>   limite por pessoa), mostradas no cartão público e no convite. Cupons
>   ganhou a aba **Controle de uso**: usos, custo e receita por tipo e cupom a
>   cupom, com "—" (nunca zero) para o que não se sabe. O custo interno do vale
...
>   rolava para o lado, e o que não cabia não era visto. Agora são duas linhas
>   com nome: **Atender** (reservas, jogo aberto, membros, aulas, torneios,
>   loja, operação) e **Gerir** (perfil, estrutura, marketing, desempenho,
>   equipe, configurações). Nada rola para o lado, nem as abas de cada seção.
>   A mesma peça (`V2SectionNav`) serve ao painel do professor e à gestão do
>   torneio. **Banco: zero.**
>
...
|---|---|---|
| **Testes Vitest** | **6059 passing** (371 arquivos) + 393 asserções de regras do Firestore no emulador (+ 17 do Storage) | +5169 (era 408) |
